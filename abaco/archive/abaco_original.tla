...
AllWorkersOfActorUseSameImageVersion_Inv == \A a \in Actors: \A x, y \in actorWorkers[a]:
...
\*    /\ Len(command_queues[a]) <  MaxQueueSize   --- TODO: what criteria to put here?
    ...
    /\ UNCHANGED<<worker_command_queues,m, workerStatus,totalNumWorkers, workersCreated, 
                  actorWorkers, currentImageVersion,currentImageVersionForWorkers,
                  actor_msg_queues,currentTotActiveWorkers,Workers>>                                                                        
...
(i.e., to update an actor definition)
*)
    ...
    /\  UNCHANGED<<worker_command_queues, workerStatus, m, totalNumWorkers, workersCreated,
                   actorWorkers, currentImageVersionForWorkers, actor_msg_queues,
                   currentTotActiveWorkers, Workers>>  
...
The enabling condition is the actorStatus value (UPDATING_IMAGE) which is set
...
*)
    ...
    /\ UNCHANGED<<actor_msg_queues,worker_command_queues,tmsg,workerStatus,m,totalNumWorkers,
                  workersCreated,actorWorkers,currentImageVersion,currentImageVersionForWorkers,
                  currentTotActiveWorkers,Workers>>
...
*)
    ...
    /\ worker_command_queues' = [worker_command_queues 
                                EXCEPT ![w] = Append(worker_command_queues[w],
                               [type |->"COMMAND", message |->"SHUTDOWN"])]
    ...
    /\ UNCHANGED<<actor_msg_queues,command_queues, actorStatus, m, tmsg, totalNumWorkers,
                  workersCreated, currentImageVersion,currentImageVersionForWorkers,
                  currentTotActiveWorkers, Workers>>                                                  
 
 StartDeleteWorkerONFinish(w,a) ==
 /\ actorStatus[a] = "UPDATING_IMAGE"
 /\ workerStatus[w].status = "FINISHED"
 /\ workerStatus[w].status # "-"
 /\ workerStatus[w].status # "SHUTDOWN_REQUESTED"
 /\ w \in actorWorkers[a]
 /\ workerStatus' = [workerStatus EXCEPT ![w]=[actor|->a, status|->"SHUTDOWN_REQUESTED"]]
...
*)
    ...
    /\ UNCHANGED<<actor_msg_queues,actorWorkers,workerStatus,command_queues,actorStatus,m, tmsg,
                  totalNumWorkers, workersCreated, currentImageVersion,currentImageVersionForWorkers,
                  currentTotActiveWorkers,Workers>>
...
*)
    ...
    /\ UNCHANGED<<actor_msg_queues, actorWorkers, command_queues,actorStatus,m, tmsg, totalNumWorkers,
                  workersCreated, currentImageVersion, currentImageVersionForWorkers,
                  Workers>>
...
*)
    ...
    /\ workerStatus' = [workerStatus EXCEPT ![w]=[actor|->a, status|->"DELETED"]]
    /\ currentTotActiveWorkers' = currentTotActiveWorkers - 1
    /\ UNCHANGED<<actor_msg_queues,command_queues,worker_command_queues,actorStatus,m, tmsg, totalNumWorkers,
...
*)
    ...
    /\ UNCHANGED<<worker_command_queues,workerStatus,m, tmsg, totalNumWorkers, workersCreated,actorWorkers, currentImageVersion,
                  currentImageVersionForWorkers,currentTotActiveWorkers,Workers>> 
...
*)
    ...
    /\ UNCHANGED<<command_queues, worker_command_queues,actorStatus,m, tmsg, totalNumWorkers, workersCreated, actorWorkers,
...
*)
    ...
    /\ workerStatus' = [workerStatus EXCEPT ![w]=[actor|->a, status|->"IDLE"]] 
    /\ UNCHANGED<<actor_msg_queues,command_queues,worker_command_queues,actorStatus,m, tmsg, totalNumWorkers,
                  workersCreated,actorWorkers, currentImageVersion,currentImageVersionForWorkers,currentTotActiveWorkers,Workers>>         
...
Next == 
    ...
    \/ \E w \in Workers, a \in Actors: StartDeleteWorkerONFinish(w,a)
...
FairSpec == Spec
        ...
        /\ WF_vars(\E w \in Workers, a \in Actors: StartDeleteWorkerONFinish(w,a))
